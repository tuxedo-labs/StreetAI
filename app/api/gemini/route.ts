...
async function getLocationDetails(lat: number, lng: number) {
  ...
  if (!data || !data.address) return { locationName: "Unknown Location", country: "Unknown", city: "Unknown" };
...
export async function POST(request: Request) {
  try {
    ...
    const { locationName, country, city } = await getLocationDetails(location.lat, location.lng);
